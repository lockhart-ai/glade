...
| 9 · Todos | The agent's checklist with progress, grouped: active, then done (a filled teal check, struck through, newest first, each with when it finished), then not started (a hollow ring; #282, #308); and each task's progress on its sidebar row's third line (a ring and `3/7`; a check once all are done), which shows only while a task has something on it. | ![9 · Todos](screens/09-todos.png) |
| 10 · Artifacts | Deliverables as compact rows, newest file first, under date headers (Today, Yesterday, This week, Last week, This month, Older) with counts that fold (older ones folded to begin with); a thumbnail for an image, a type tile for anything else, then title, type and age; a hovered row's Open, Reveal in folder and More in place of its age; the file showing in Files outlined (#307). | ![10 · Artifacts](screens/10-artifacts.png) |
| 11 · Subagents | Status and latest line per subagent, and under a running one's name the SDK's one-line summary of what it's doing now (#278); one expanded inline to its log, with the background work it left running under it and an eye and count on its row (#291). The running ones' count on the task's sidebar row, on its third line. | ![11 · Subagents](screens/11-subagents.png) |
...
| 28 · Watchers | The Watchers tab: what the agent left running or scheduled (Monitor watches, background commands, wakeups, cron jobs), each with its state, what it runs, its last output, how many times it woke the agent and when, and Stop while it is live; the count of live ones on the tab. Only the task's own: a subagent's are under it in the Subagents tab (#291). The eye and count on a sidebar row's third line, after any todo progress and running subagents: a task with live watchers, done or not. Built from the Subagents tab; no original design. | ![28 · Watchers](screens/28-watchers.png) |
