...
| 9 · Todos | The agent's checklist with progress, grouped: active, then done (a filled teal check, struck through, newest first, each with when it finished), then not started (a hollow ring; #282, #308); and each task's progress on its sidebar row (a ring and `3/7`; a check once all are done). | ![9 · Todos](screens/09-todos.png) |
