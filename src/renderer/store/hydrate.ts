 ...
 * tabs, every task's live watchers and running subagents, the UI state, the settings and the models the pickers
 * offer, with the selection restored. The selected task is loaded wherever it is, and the
...
export async function loadSnapshot(bridge: GladeBridge): Promise<GladeData> {
  const [{ workspaces }, { entries }, { tabs: terminalTabs }, { settings }, { watchers }, { calls }, { models }] =
    await Promise.all([
