 ...
 * tabs, every task's live watchers, the UI state, the settings and the account, with the selection restored. The selected task is loaded wherever it is, and the
...
export async function loadSnapshot(bridge: GladeBridge): Promise<GladeData> {
  const [{ workspaces }, { entries }, { tabs: terminalTabs }, { settings }, { watchers }, { status: accountStatus }] =
    await Promise.all([
