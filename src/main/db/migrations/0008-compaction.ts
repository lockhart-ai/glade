...
/**
 * Adds compactions to the tool log (`CompactionEvent` in `src/shared/domain.ts`): a `compaction` kind, with its
 * trigger, its state (in `tool_state`, as a tool call's), the tokens before and after, and the context window at the
 * time. SQLite can't change a table's CHECK constraints, so the table is rebuilt: copied into a new one with the new
 * kind and columns, then swapped in. Nothing references `tool_events`, so dropping the old one is safe.
 */
export const compactionMigration: Migration = {
  version: 8,
  name: 'Add compactions to the tool log',
  up(db) {
    db.exec(`
      ...
      INSERT INTO tool_events_new (id, task_id, seq, kind, turn, created_at, text, tool_name, tool_input, tool_output,
        tool_state, tool_use_id, parent_tool_use_id, divider_kind)
      SELECT id, task_id, seq, kind, turn, created_at, text, tool_name, tool_input, tool_output, tool_state,
        tool_use_id, parent_tool_use_id, divider_kind
      FROM tool_events;
