  ...
  it('rejects an invalid request with a typed error, leaving the database alone', async () => {
    ...
    await expect(glade.invoke(CommandName.UiStateSet, request)).rejects.toEqual(
      bridgeError(
        ...
        'uiState.set: key: Invalid option: expected one of "active_workspace_id"|"selected_task_id"|"pinned_section_collapsed"|"active_section_collapsed"|"done_section_collapsed"|"task_filter"|"relaunch_notice"|"right_panel_tab"|"right_panel_width"|"right_panel_collapsed"|"sidebar_collapsed"|"sidebar_width"|"bottom_bar_collapsed"|"bottom_bar_height"|"plugin_width"|"terminal_tab"; value: Invalid input: expected string, received number',
      ),
    )
