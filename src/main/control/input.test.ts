...
const BAD_INPUTS: readonly BadInput[] = [
  ...
  [ControlToolName.CreateTask, () => ({ workspaceId, model: 'gpt-5' }), 'model: gpt-5 is not a model Glade offers'],
  ...
  [
    ControlToolName.UpdateTask,
    () => ({ id: task.id, patch: { model: 'nope' } }),
    'patch.model: nope is not a model Glade offers',
  ],
